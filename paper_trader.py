...
def tick():
    ...
    # 1) 이전 포지션의 시세변동 수익 반영
    if st["prev_price"]:
        ret_krw = 0.0
        for c in COINS:
            if c in st["prev_price"] and st["prev_price"][c] > 0:
                pr = price[c]/st["prev_price"][c] - 1
                ret_krw += st["positions"].get(c, 0.0) * pr
        # 캐리 펀딩(경과시간 비례)
        hrs = 0.5
        if st["last_tick"]:
            hrs = min(8, (now - dt.datetime.fromisoformat(st["last_tick"])).total_seconds()/3600)
        ret_krw += carry_funding(eq) * C_FUND_8H * (hrs/8)
        eq += ret_krw

    # 2) 새 목표 계산
    tt = turtle_targets(h4, d1, price, eq, st)
    xm = momentum_targets(d1, eq, st)
    net = {c: tt.get(c,0)+xm.get(c,0) for c in COINS}   # 캐리는 델타중립이라 가격노출 net 제외

    # 3) 목표 변화 → 거래비용 차감 + 매매일지
    for c in COINS:
        old = st["positions"].get(c, 0.0)
        if abs(net[c]-old) > eq*0.005:   # 0.5% 이상 변화만 거래로 기록
            delta = net[c]-old
            eq -= abs(delta)*COST
            st["trades"].append({"t": now.strftime("%Y-%m-%d %H:%M"), "coin": c,
                                 "act": ("롱↑" if delta>0 else "숏↑") if abs(net[c])>abs(old) else "축소/청산",
                                 "notional": round(net[c]), "price": round(price[c])})
    st["trades"] = st["trades"][-100:]   # 최근 100건
